...
*  Allow users to send and receive messages to/from each other;
   The system should provde a way to interchange text messages between two
   users that are registered (logged-in) the service. 
   The comunication between two users is always encrypted.
   
*  Allow users to send messages in named groups;
   The system provides a way to creat groups where messages can be interchanges
   between many users. Also the communication between all the users in a
   certain group should be encrypted by defatul. Chat groups can be open (in
...
hundreths of plugins that allows to build a web application as if it was a
...
interconected subsystems, so that is possible to scale the system vertically if
needed. Also the system should use asynchrnous mechanisms to write/retrive data
...
render templates, menus, and formularies according to the screen flow of the
messaging system.
...
The Relational DB can be just a basic linux service running MySQL. 
 ...
 * DM message page with an integrated search form/dialogue?
 ...
 * GET /user
   ...
   It allows URL encoded queries to define a search according to certian
   attribtues.
 ...
 * GET /user/<user_name>
   ...
   public key that should be used to sign/encrypt or negotiate for a symetric key.
 ...
 * GET /group
   Retrives a list of chat groups, associated in some way. It allows for url
   encoded parameters to refine the search.

 * GET /group/<group_id>
   Equivalent to /user/<user_name> but for groups. All groups have assigned a
   public/private key that is generated by the backend. When requesting this
 ...
 * POST /chat/<chat_id>
   ...
   the target, which coudl be either a group or a user.
 * PUT /chat/<chat_id>
...
== Current Prototype
The current prototype, is monolitic, it doesn't allow multiple
...
idea of the datamodel, defines some concepts and ideas, the business logic of
the application, and the user interface. 
It has served to learn more about the possiblities of flask and to check the
...
achieve this and distribute this work among different memebrs of a virtual
...
* Define the protocol used to encrypt the messages: use asymetric encryption or
  symetric encryption with some interchange mechanism?
...
* Implement the javascript necesary to encrypt the messages accordingo to the
  deifned protocol.
...
* Create a dockerfile to run the fron-end, integrate with the rest of the
  system with docker compose.

* Make some UI tests, perhaps using sikulla?


=== Estimation
The items educed ealier are subjected to change and they are not providing
...
prioritizing this backlog so that a the end the team can use relative
