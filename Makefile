...
.PHONY: all clean clean_db tests
all: build
...
tests: 
	docker run -v ${SOURCE_DIR}:${TARGET_DIR} -p 8080:8080 chatservice env PYTHONPATH=${TARGET_DIR}/src:${TARGET_DIR}/src/lib  nosetests -v --all-modules ${TARGET_DIR}/tests
